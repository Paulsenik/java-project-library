...
public class PUIRotaryControl extends PUIElement {
  ...
  private void init() {
    paint = (g, x, y, w, h) -> {
      ...
      // Value-Visual
      g.fillArc(x, y, w, h, (int) (360 - ((rotationArea * value + (360 - rotationArea) / 2 + 90)
          + valueThickness / 2)), (int) valueThickness);

      // Overpaint part of ^ , to visualize valueLength
      g.setColor(getBackgroundColor());

      g.fillOval((int) (x + (1.0f - valueLength) * (w / 2)),
          (int) (y + (1.0f - valueLength) * (h / 2)), (int) (w * (valueLength)),
          (int) (h * (valueLength)));
