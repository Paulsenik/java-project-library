      ...
      if (fixedElements) { // snap to grid
        for (int i = 0; i < (Math.min(elements.size(), showedElements))
          ...
          if (alignment == ElementAlignment.VERTICAL) {
            ...
            // Vertical
            e.setBounds(x + elementSpace_Left, (int) (y + eHeight * i) + elementSpace_Top,
                w - (showSlider ? sliderWidth : 0) - elementSpace_Left - elementSpace_Right,
                (int) eHeight - elementSpace_Top - elementSpace_Bottom);

          } else if (alignment == ElementAlignment.HORIZONTAL) {

            // Horizontal
            e.setBounds((int) (x + eWidth * i + elementSpace_Left), y + elementSpace_Top,
                (int) eWidth - elementSpace_Left - elementSpace_Right,
                h - (showSlider ? sliderWidth : 0) - elementSpace_Top - elementSpace_Bottom);
    ...
    if (element != null) {
      for (int i = 0; i < elements.size(); i++) {
        if (elements.get(i) == element) {
          return Math.min(
              (Math.max(i - showedElements / 2, 0f)) / (elements.size() - showedElements), 1f);
        }
      }
    }
