...
/**
 * Automatically scales and positions its Elements Can have an 2DArray of Elements that are then
 * transformed into a matrix-pattern
 */
      ...
      for (int i = 0; i < elements.length; i++) {
        for (int j = 0; j < elements[i].length; j++) {
          if (elements[i][j] != null) {
            elements[i][j].setBounds((int) (eW * i + x) + elementSpace_Left,
                (int) (eH * j + y) + elementSpace_Top,
                (int) (eW - elementSpace_Left - elementSpace_Right),
                (int) (eH - elementSpace_Top - elementSpace_Bottom));
          }
        }
      }
      frame.repaint();
